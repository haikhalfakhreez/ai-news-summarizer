...
export const Home: NextPage = () => {
  ...
  useEffect(() => {
    if (
      ...
      generateSummary('https://productnation.co/' + (urlState as string[]).join('/'))
    }
  ...
  const generateSummary = async (url?: string) => {
    setSummary('')
    if (url) {
      if (!url.includes('productnation.co')) {
        toast.error('Please enter a valid ProductNation article')
        return
      }
      setCurArticle(url)
    } else {
      if (!curArticle.includes('productnation.co')) {
        toast.error('Please enter a valid ProductNation article')
        return
      }
          ...
          <div className="mb-10 px-4">
            ...
            <div className="mx-auto mt-6 max-w-3xl text-lg leading-7">
              {summary.split('. ').map((sentence, index) => (
                <div key={index}>
                  {sentence.length > 0 && <li className="mb-2 list-disc">{sentence}</li>}
                </div>
              ))}
            </div>
          </div>
